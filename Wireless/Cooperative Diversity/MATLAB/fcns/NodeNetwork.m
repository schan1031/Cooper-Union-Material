classdef NodeNetwork < handle
    ...
    methods
        function obj = NodeNetwork(pos,power,fc,fs,fd,noisefloor,shadow)
            ...
            obj.fs = fs;
            obj.fc = fc;
            obj.likelihood = ones(obj.nodenum,obj.nodenum,2);
            obj.likelihoodflag = 0;
        ...
        function [ ] = direct_tx(obj,tx_node,rx_node,msg)
            disp([num2str(tx_node) ' --> ' num2str(rx_node)]);
        ...
        function [ ] = alamouti_tx(obj, tx_node1, tx_node2, msg)
            disp([num2str(tx_node1) ' & ' num2str(tx_node2) '--> ALL']);
            tx_crc = step(obj.hCRC,msg);
            signal = step(obj.hmod,tx_crc);
            sig0 = signal(1:2:end);
            sig1 = signal(2:2:end);
        ...
        function [ status, div_node ] = adaptive_tx(obj,tx_node,dest_node,msg)
            ...
            if obj.errarray(dest_node) == 0
                ...
                if isempty(candidates)
                    disp('No diversity candidates found.');
                    status = -1;
                    return;
                else
                    if length(candidates) == 1
                        disp(['Found: node ' num2str(candidates)]);
                        div_node = candidates;
                    else
                        disp(['Found: nodes ' num2str(candidates')]);
                        if obj.likelihoodflag == 0
                            ii = randi(length(candidates),1,1);
                            div_node = candidates(ii);
                            disp(['Selected node ' num2str(div_node) ' randomly.']);
                        elseif obj.likelihoodflag == 1
                            maxii = 0;
                            maxll = 0;
                            for ii = 1:length(candidates)
                                if obj.likelihood(tx_node,candidates(ii),1) > maxll
                                    maxll = obj.likelihood(tx_node,candidates(ii),1);
                                    maxii = ii;
                                end
                            end
                            div_node = candidates(maxii);
                            disp(['Selected node ' num2str(div_node) ' using network information.']);
                        end
                    end
                ...
                if obj.errarray(dest_node) == 0
                    disp(['Success.']);
                    status = 2;
                    if obj.likelihoodflag == 1
                        ll = obj.likelihood(tx_node,div_node,1);
                        n = obj.likelihood(tx_node,div_node,2);
                        obj.likelihood(tx_node,div_node,1) = ll*(n/(n+1)) + 1/(n+1);
                        obj.likelihood(tx_node,div_node,2) = n+1;
                    end
                else
                    disp(['Failed.']);
                    status = -2;
                    if obj.likelihoodflag == 1
                        ll = obj.likelihood(tx_node,div_node,1);
                        n = obj.likelihood(tx_node,div_node,2);
                        obj.likelihood(tx_node,div_node,1) = ll*(n/(n+1));
                        obj.likelihood(tx_node,div_node,2) = n+1;
                    end
                end
            end
        ...
        function [ h ] = topology(obj,offset)
            h = figure;
            grid on;
            ...
            text(obj.nodepos(:,1)+offset,obj.nodepos(:,2),num2str((1:length(obj.nodepos))'))
        ...
        function [ ] = single_trial(obj,tx_node,rx_node,trials)
            ...
            for trial = 1:trials
                ...
                obj.channelarray{tx_node,rx_node}.reset();
                obj.direct_tx(tx_node,rx_node,msg);
        ...
        function [ ] = gen_likelihood(obj,tx_node,div_node,status)
            if length(div_node) ~= 1
                for node = 1:obj.nodenum
                    if node == tx_node
                        obj.likelihood(tx_node,node,1) = NaN;
                        obj.likelihood(tx_node,node,1) = NaN;
                    end
                    obj.likelihood(tx_node,node,1) = sum(status(div_node == node) > 0) / sum(div_node == node);
                    obj.likelihood(tx_node,node,2) = sum(div_node == node);
                end
            else
                for node = 1:obj.nodenum
                    if node == tx_node
                    ...
                    obj.likelihood(tx_node,node,1) = 1;
                    obj.likelihood(tx_node,node,2) = 1;
                end
            end
        end
