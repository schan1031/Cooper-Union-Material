...
tremolo:
	...
	clr x1
	move a,x0
	move x:Knob_2,x1						; Knob 2 is frequency
...
;										create wave table for LFO
;	move 	x:Knob_1,x0					;take knob one value, depth factor
;	move	x:Knob_2,x0					;second knob as variable LFO frequency
;	mpy 	x0,b						;Multiply depth factor by sample copy

;Knob_Coefficient
	;Convert Knob 1 to Frequency
;        move    x:Knob_1,x0                             ; load Knob 1's current value (range = 0 to 1)
;        move    #>$199999,a                             ;  a =   1 Hz
;        move    #>$7FFFFF,y0                            ; y0 = 10 Hz
;        mac     x0,y0,a                                 ; a  =  10 * value + 1; register a now ranges between 50 and 5000 Hz
;        move    x0,x:Filter_1_K                          ; store current value
;		move	x0,x:Debug_Read_from_DSP_1				; DEBUG Example, send filter 1 frequency coefficient to GUI

;Knob_Coefficient
	;Convert Knob 1 to Frequency
;        move    x:Knob_1,x1                             ; load Knob 1's current value (range = 0 to 1)
;        move    #>$199999,a                             ;  a =   1 Hz
;        move    #>$7FFFFF,y0                            ; y0 = 10 Hz
;        mac     x0,y0,a                                 ; a  =  10 * value + 1; register a now ranges between 50 and 5000 Hz
;        move    x0,x1                         			 ; store current value
;		move	x0,x:Debug_Read_from_DSP_1				; DEBUG Example, send filter 1 frequency coefficient to GUI

;Knob_Coefficient
;	;Convert Knob 2 to Frequency
;        move    x:Knob_2,x0                             ; load Knob 1's current value (range = 0 to 1)
;        move    #>$199999,a                             ;  a =   1 Hz
;        move    #>$7FFFFF,y0                            ; y0 = 10 Hz
;        mac     x0,y0,a                                 ; a  =  10 * value + 1; register a now ranges between 50 and 5000 Hz
